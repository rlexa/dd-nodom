...
| `arrayExtractDistinctValuesOfKey` | `key => [] => []`              | returns distinct values of a certain key from all items                   |
...
| `arrayReduce`                     | `fnInit => fn => [] => acc`    | [reduces array via `fn: (acc, item, index, all) => accNew`](#arrayreduce) |
